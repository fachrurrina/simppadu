...
class M_sik extends CI_Model{
	...
	public function get_like_no_sk($no_sk){
		...
		$sql = "select * from t_sik where no_sk like '%$no_sk%'";
	...
	public function set_status_berlaku($no_daftar, $status_berlaku){
		$this->db->where('no_daftar', $no_daftar);
		$update = $this->db->update('t_sik', array(
	...
	public function get_where_no_sk($no_sk){
		...
		$sql = "select * from t_sik where no_sk = '$no_sk' and status_berlaku = 1";
		$query = $this->db->query($sql);
		if($query->num_rows() > 0){
	...
	public function get_where_no_sk_2($no_sk){
		...
		$sql = "select * from t_sik where no_sk = '$no_sk'";
		$query = $this->db->query($sql);
		if($query->num_rows() > 0){
	...
	public function get_no_urut_baru(){
		 ...
		 */
		$sql = "SELECT * FROM t_sik ORDER BY no_urut DESC LIMIT 1"; 
		$query = $this->db->query($sql);
	...
	public function simpan_baru_operator($no_daftar, $array_update){
		$this->db->where('no_daftar', $no_daftar);
		$update = $this->db->update('t_sik', $array_update);
		return ($update) ? true : false;
	}

	public function cetak_sik_baru($no_daftar){

		$this->load->library('phpword');
        $template = $this->phpword->loadTemplate(APPPATH ."../templates/sik_baru.docx");

        $data  = $this->m_sik->get_where_no_daftar($no_daftar);
        ...
        $template->setValue('nama_klinik', $data->nama_klinik);
        $template->setValue('alamat_lengkap_klinik', ucwords(strtolower($data->alamat_klinik .' Gp. '. $this->m_kel->get_nm_kel($data->id_kel_klinik) .' Kecamatan '. $this->m_kec->get_nm_kec($data->id_kec_klinik))));
        $template->setValue('jasa_pelayanan', $data->jasa_pelayanan);
        $template->setValue('nama_pimpinan', $data->nama_pimpinan);
        ...
        $template->save(APPPATH. '../saved/sik_baru.docx');
        $this->m_fo->set_status_proses($no_daftar, 11);




        $template_sk = $this->phpword->loadTemplate(APPPATH ."../templates/sik_baru_sk.docx");
        ...
        $template_sk->setValue('nama_klinik', $data->nama_klinik);
        $template_sk->setValue('alamat_lengkap_klinik', ucwords(strtolower($data->alamat_klinik .' Gp. '. $this->m_kel->get_nm_kel($data->id_kel_klinik) .' Kecamatan '. $this->m_kec->get_nm_kec($data->id_kec_klinik))));
        $template_sk->setValue('jasa_pelayanan', $data->jasa_pelayanan);
        $template_sk->setValue('nama_pimpinan', $data->nama_pimpinan);
        ...
        $template_sk->save(APPPATH. '../saved/sik_baru_sk.docx');
        $this->m_fo->set_status_proses($no_daftar, 11);
        ...
        <script>
            ...
            window.location = 'http://<?php echo $_SERVER['SERVER_NAME'] ?>/simppadu/saved/sik_baru.docx';
            setTimeout(function(){
            	window.location = 'http://<?php echo $_SERVER['SERVER_NAME'] ?>/simppadu/saved/sik_baru_sk.docx';
            	setTimeout(function(){
	...
	public function cetak_sik_perpanjangan($no_daftar){

		$this->load->library('phpword');
        $template = $this->phpword->loadTemplate(APPPATH ."../templates/sik_perpanjangan.docx");
        ...
        $template->setValue('value9', ucwords(strtolower($this->m_bidang_sik->get_nama_bidang_sik($data->id_bidang_sik))));
        ...
        $template->save(APPPATH. '../saved/sik_perpanjangan.docx');
        $this->m_fo->set_status_proses($no_daftar, 11);

        ?>
        <script>
            window.location = 'http://<?php echo $_SERVER['SERVER_NAME'] ?>/simppadu/saved/sik_perpanjangan.docx';
            setTimeout(function(){
            	window.location = '<?php echo site_url("c_cetak/belum") ?>';
            }, 3000);
        </script>
	...
	public function cetak_sik_perubahan($no_daftar){

		$this->load->library('phpword');
        $template = $this->phpword->loadTemplate(APPPATH ."../templates/sik_perubahan.docx");
        ...
        $template->save(APPPATH. '../saved/sik_perubahan.docx');
        $this->m_fo->set_status_proses($no_daftar, 11);

        ?>
        <script>
            window.location = 'http://<?php echo $_SERVER['SERVER_NAME'] ?>/simppadu/saved/sik_perubahan.docx';
            setTimeout(function(){
            	window.location = '<?php echo site_url("c_cetak/belum") ?>';
            }, 3000);
        </script>
