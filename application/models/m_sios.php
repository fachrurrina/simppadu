...
class M_sios extends CI_Model{
	...
	public function get_like_no_sk($no_sk){
		...
		$sql = "select * from t_sios where no_sk like '%$no_sk%'";
	...
	public function set_status_berlaku($no_daftar, $status_berlaku){
		$this->db->where('no_daftar', $no_daftar);
		$update = $this->db->update('t_sios', array(
	...
	public function get_where_no_sk($no_sk){
		...
		$sql = "select * from t_sios where no_sk = '$no_sk' and status_berlaku = 1";
		$query = $this->db->query($sql);
		if($query->num_rows() > 0){
	...
	public function get_where_no_sk_2($no_sk){
		...
		$sql = "select * from t_sios where no_sk = '$no_sk'";
		$query = $this->db->query($sql);
		if($query->num_rows() > 0){
	...
	public function get_no_urut_baru(){
		 ...
		 */
		$sql = "SELECT * FROM t_sios ORDER BY no_urut DESC LIMIT 1"; 
		$query = $this->db->query($sql);
	...
	public function simpan_baru_operator($no_daftar, $array_update){
		$this->db->where('no_daftar', $no_daftar);
		$update = $this->db->update('t_sios', $array_update);
		return ($update) ? true : false;
	}

	public function cetak_sios_baru($no_daftar){

		$this->load->library('phpword');
        

        $data  = $this->m_sios->get_where_no_daftar($no_daftar);
        $fo  = $this->m_fo->get_where_no_daftar($no_daftar);

        $template = $this->phpword->loadTemplate(APPPATH ."../templates/sios_baru.docx");
        ...
        $template->setValue('alamat_lengkap_rumah_sakit', ucwords(strtolower($data->alamat_rumah_sakit .' Gp. '. $this->m_kel->get_nm_kel($data->id_kel_rumah_sakit) .' Kec. '. $this->m_kec->get_nm_kec($data->id_kec_rumah_sakit))));
        ...
        $template->save(APPPATH. '../saved/sios_baru.docx');

        $template_sk = $this->phpword->loadTemplate(APPPATH ."../templates/sios_baru_sk.docx");
        $template_sk->setValue('no_sk', $data->no_sk);
        $template_sk->setValue('nama_pemohon', $fo->nama_pemohon);
        $template_sk->setValue('tanggal_daftar', convert_tanggal_jadi_string($fo->tanggal_daftar));

        

        $template_sk->setValue('nama_rumah_sakit', $data->nama_rumah_sakit);
        ...
        $template_sk->setValue('nama_rumah_sakit', $data->nama_rumah_sakit);
        $template_sk->setValue('nama_rumah_sakit_judul', $data->nama_rumah_sakit);
        $template_sk->setValue('alamat_lengkap_rumah_sakit', ucwords(strtolower($data->alamat_rumah_sakit .' Gp. '. $this->m_kel->get_nm_kel($data->id_kel_rumah_sakit) .' Kec. '. $this->m_kec->get_nm_kec($data->id_kec_rumah_sakit))));

        $template_sk->setValue('dokter_penanggung_jawab', $data->dokter_penanggung_jawab);
        $template_sk->setValue('no_sip_dokter', $data->no_sip_dokter);
        $template_sk->setValue('tanggal_berlaku_sip', convert_tanggal_jadi_string($data->tanggal_berlaku_sip));
        ...
        $template_sk->save(APPPATH. '../saved/sios_baru_sk.docx');
        $this->m_fo->set_status_proses($no_daftar, 11);

        ?>
        <script>
            
            window.location = 'http://<?php echo $_SERVER['SERVER_NAME'] ?>/simppadu/saved/sios_baru.docx';
            setTimeout(function(){
            	window.location = 'http://<?php echo $_SERVER['SERVER_NAME'] ?>/simppadu/saved/sios_baru_sk.docx';
            	setTimeout(function(){
	...
	public function cetak_sios_perpanjangan($no_daftar){

		$this->load->library('phpword');
        $template = $this->phpword->loadTemplate(APPPATH ."../templates/sios_perpanjangan.docx");
        ...
        $template->setValue('value9', ucwords(strtolower($this->m_bidang_sios->get_nama_bidang_sios($data->id_bidang_sios))));
        ...
        $template->save(APPPATH. '../saved/sios_perpanjangan.docx');
        $this->m_fo->set_status_proses($no_daftar, 11);

        ?>
        <script>
            window.location = 'http://<?php echo $_SERVER['SERVER_NAME'] ?>/simppadu/saved/sios_perpanjangan.docx';
            setTimeout(function(){
            	window.location = '<?php echo site_url("c_cetak/belum") ?>';
            }, 3000);
        </script>
	...
	public function cetak_sios_perubahan($no_daftar){

		$this->load->library('phpword');
        $template = $this->phpword->loadTemplate(APPPATH ."../templates/sios_perubahan.docx");
        ...
        $template->save(APPPATH. '../saved/sios_perubahan.docx');
        $this->m_fo->set_status_proses($no_daftar, 11);

        ?>
        <script>
            window.location = 'http://<?php echo $_SERVER['SERVER_NAME'] ?>/simppadu/saved/sios_perubahan.docx';
            setTimeout(function(){
            	window.location = '<?php echo site_url("c_cetak/belum") ?>';
            }, 3000);
        </script>
