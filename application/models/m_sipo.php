...
class M_sipo extends CI_Model{
	...
	public function get_like_no_sk($no_sk){
		...
		$sql = "select * from t_sipo where no_sk like '%$no_sk%'";
	...
	public function set_status_berlaku($no_daftar, $status_berlaku){
		$this->db->where('no_daftar', $no_daftar);
		$update = $this->db->update('t_sipo', array(
	...
	public function get_where_no_sk($no_sk){
		...
		$sql = "select * from t_sipo where no_sk = '$no_sk' and status_berlaku = 1";
		$query = $this->db->query($sql);
		if($query->num_rows() > 0){
	...
	public function get_where_no_sk_2($no_sk){
		...
		$sql = "select * from t_sipo where no_sk = '$no_sk'";
		$query = $this->db->query($sql);
		if($query->num_rows() > 0){
	...
	public function get_no_urut_baru(){
		 ...
		 */
		$sql = "SELECT * FROM t_sipo ORDER BY no_urut DESC LIMIT 1"; 
		$query = $this->db->query($sql);
	...
	public function simpan_baru_operator($no_daftar, $array_update){
		$this->db->where('no_daftar', $no_daftar);
		$update = $this->db->update('t_sipo', $array_update);
		return ($update) ? true : false;
	}

	public function cetak_sipo_baru($no_daftar){

		$this->load->library('phpword');
        $template = $this->phpword->loadTemplate(APPPATH ."../templates/sipo_baru.docx");

        $data  = $this->m_sipo->get_where_no_daftar($no_daftar);
        ...
        $template->setValue('no_surat_izin_kerja', $data->no_surat_izin_kerja);

        $template->setValue('nama_optik', $data->nama_optik);
        $template->setValue('alamat_lengkap_optik', ucwords(strtolower($data->alamat_optik .' Gp. '. $this->m_kel->get_nm_kel($data->id_kel_optik) .' Kec. '. $this->m_kec->get_nm_kec($data->id_kec_optik))));
        ...
        $template->save(APPPATH. '../saved/sipo_baru.docx');
        $this->m_fo->set_status_proses($no_daftar, 11);

        ?>
        <script>
            window.location = 'http://<?php echo $_SERVER['SERVER_NAME'] ?>/simppadu/saved/sipo_baru.docx';
            setTimeout(function(){
	...
	public function cetak_sipo_perpanjangan($no_daftar){

		$this->load->library('phpword');
        $template = $this->phpword->loadTemplate(APPPATH ."../templates/sipo_perpanjangan.docx");
        ...
        $template->setValue('alamat_lengkap_optik', ucwords(strtolower($data->alamat_optik .' Gp. '. $this->m_kel->get_nm_kel($data->id_kel_optik) .' Kecamatan '. $this->m_kec->get_nm_kec($data->id_kec_optik))));
        
        $template->setValue('tanggal_perpanjangan', convert_tanggal_jadi_string($data->tanggal_perpanjangan));
        $template->setValue('tanggal_terbit', convert_tanggal_jadi_string($data->tanggal_terbit));


        $template->save(APPPATH. '../saved/sipo_perpanjangan.docx');
        $this->m_fo->set_status_proses($no_daftar, 11);

        ?>
        <script>
            window.location = 'http://<?php echo $_SERVER['SERVER_NAME'] ?>/simppadu/saved/sipo_perpanjangan.docx';
            setTimeout(function(){
            	window.location = '<?php echo site_url("c_cetak/belum") ?>';
            }, 3000);
        </script>
	...
	public function cetak_sipo_perubahan($no_daftar){

		$this->load->library('phpword');
        $template = $this->phpword->loadTemplate(APPPATH ."../templates/sipo_perubahan.docx");
        ...
        $template->setValue('value9', ucwords(strtolower($this->m_bidang_sipo->get_nama_bidang_sipo($data->id_bidang_sipo))));
        ...
        $template->save(APPPATH. '../saved/sipo_perubahan.docx');
        $this->m_fo->set_status_proses($no_daftar, 11);

        ?>
        <script>
            window.location = 'http://<?php echo $_SERVER['SERVER_NAME'] ?>/simppadu/saved/sipo_perubahan.docx';
            setTimeout(function(){
            	window.location = '<?php echo site_url("c_cetak/belum") ?>';
            }, 3000);
        </script>
