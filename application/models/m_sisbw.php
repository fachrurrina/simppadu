...
class M_sisbw extends CI_Model{
	...
	public function get_like_no_sk($no_sk){
		...
		$sql = "select * from t_sisbw where no_sk like '%$no_sk%'";
	...
	public function set_status_berlaku($no_daftar, $status_berlaku){
		$this->db->where('no_daftar', $no_daftar);
		$update = $this->db->update('t_sisbw', array(
	...
	public function get_where_no_sk($no_sk){
		...
		$sql = "select * from t_sisbw where no_sk = '$no_sk' and status_berlaku = 1";
		$query = $this->db->query($sql);
		if($query->num_rows() > 0){
	...
	public function get_where_no_sk_2($no_sk){
		...
		$sql = "select * from t_sisbw where no_sk = '$no_sk'";
		$query = $this->db->query($sql);
		if($query->num_rows() > 0){
	...
	public function get_no_urut_baru(){
		 ...
		 */
		$sql = "SELECT * FROM t_sisbw ORDER BY no_urut DESC LIMIT 1"; 
		$query = $this->db->query($sql);
	...
	public function simpan_baru_operator($no_daftar, $array_update){
		$this->db->where('no_daftar', $no_daftar);
		$update = $this->db->update('t_sisbw', $array_update);
		return ($update) ? true : false;
	}

	public function cetak_sisbw_baru($no_daftar){

		$this->load->library('phpword');
        $template = $this->phpword->loadTemplate(APPPATH ."../templates/sisbw_baru.docx");

        $data  = $this->m_sisbw->get_where_no_daftar($no_daftar);
        ...
        $template->setValue('npwpd_npwrd', $data->npwpd_npwrd);
        $template->setValue('alamat_lengkap_usaha', ucwords(strtolower($data->alamat_usaha .' Gp. '. $this->m_kel->get_nm_kel($data->id_kel_usaha) .' Kecamatan '. $this->m_kec->get_nm_kec($data->id_kec_usaha))));
        $template->setValue('no_telp', $data->no_telp);
        $template->setValue('klasifikasi', $data->klasifikasi);
        ...
        $template->save(APPPATH. '../saved/sisbw_baru.docx');
        $this->m_fo->set_status_proses($no_daftar, 11);

        ?>
        <script>
            window.location = 'http://<?php echo $_SERVER['SERVER_NAME'] ?>/simppadu/saved/sisbw_baru.docx';
            setTimeout(function(){
	...
	public function cetak_sisbw_perpanjangan($no_daftar){

		$this->load->library('phpword');
        $template = $this->phpword->loadTemplate(APPPATH ."../templates/sisbw_perpanjangan.docx");
        ...
        $template->setValue('value9', ucwords(strtolower($this->m_bidang_sisbw->get_nama_bidang_sisbw($data->id_bidang_sisbw))));
        ...
        $template->save(APPPATH. '../saved/sisbw_perpanjangan.docx');
        $this->m_fo->set_status_proses($no_daftar, 11);

        ?>
        <script>
            window.location = 'http://<?php echo $_SERVER['SERVER_NAME'] ?>/simppadu/saved/sisbw_perpanjangan.docx';
            setTimeout(function(){
            	window.location = '<?php echo site_url("c_cetak/belum") ?>';
            }, 3000);
        </script>
	...
	public function cetak_sisbw_perubahan($no_daftar){

		$this->load->library('phpword');
        $template = $this->phpword->loadTemplate(APPPATH ."../templates/sisbw_perubahan.docx");
        ...
        $template->save(APPPATH. '../saved/sisbw_perubahan.docx');
        $this->m_fo->set_status_proses($no_daftar, 11);

        ?>
        <script>
            window.location = 'http://<?php echo $_SERVER['SERVER_NAME'] ?>/simppadu/saved/sisbw_perubahan.docx';
            setTimeout(function(){
            	window.location = '<?php echo site_url("c_cetak/belum") ?>';
            }, 3000);
        </script>
