...
class M_sipb extends CI_Model{
	...
	public function get_like_no_sk($no_sk){
		...
		$sql = "select * from t_sipb where no_sk like '%$no_sk%'";
	...
	public function set_status_berlaku($no_daftar, $status_berlaku){
		$this->db->where('no_daftar', $no_daftar);
		$update = $this->db->update('t_sipb', array(
	...
	public function get_where_no_sk($no_sk){
		...
		$sql = "select * from t_sipb where no_sk = '$no_sk' and status_berlaku = 1";
		$query = $this->db->query($sql);
		if($query->num_rows() > 0){
	...
	public function get_where_no_sk_2($no_sk){
		...
		$sql = "select * from t_sipb where no_sk = '$no_sk'";
		$query = $this->db->query($sql);
		if($query->num_rows() > 0){
	...
	public function get_no_urut_baru(){
		 ...
		 */
		$sql = "SELECT * FROM t_sipb ORDER BY no_urut DESC LIMIT 1"; 
		$query = $this->db->query($sql);
	...
	public function simpan_baru_operator($no_daftar, $array_update){
		$this->db->where('no_daftar', $no_daftar);
		$update = $this->db->update('t_sipb', $array_update);
		return ($update) ? true : false;
	}

	public function cetak_sipb_baru($no_daftar){

		$this->load->library('phpword');
        $template = $this->phpword->loadTemplate(APPPATH ."../templates/sipb_baru.docx");

        $data  = $this->m_sipb->get_where_no_daftar($no_daftar);
        ...
        $template->setValue('alamat_pemilik', ucwords(strtolower($data->alamat_pemilik .' Gp. '. $this->m_kel->get_nm_kel($data->id_kel_pemilik) .' Kecamatan '. $this->m_kec->get_nm_kec($data->id_kec_pemilik))));
        ...
        $template->setValue('alamat_tempat_praktek', ucwords(strtolower($data->alamat_tempat_praktek .' Gp. '. $this->m_kel->get_nm_kel($data->id_kel_tempat_praktek) .' Kecamatan '. $this->m_kec->get_nm_kec($data->id_kec_tempat_praktek))));
        $template->setValue('alamat_tempat_praktek', $data->alamat_tempat_praktek);
        ...
        $template->save(APPPATH. '../saved/sipb_baru.docx');
        $this->m_fo->set_status_proses($no_daftar, 11);

        ?>
        <script>
            window.location = 'http://<?php echo $_SERVER['SERVER_NAME'] ?>/simppadu/saved/sipb_baru.docx';
            setTimeout(function(){
	...
	public function cetak_sipb_perpanjangan($no_daftar){

		$this->load->library('phpword');
        $template = $this->phpword->loadTemplate(APPPATH ."../templates/sipb_perpanjangan.docx");
        ...
        $template->setValue('value9', ucwords(strtolower($this->m_bidang_sipb->get_nama_bidang_sipb($data->id_bidang_sipb))));
        ...
        $template->save(APPPATH. '../saved/sipb_perpanjangan.docx');
        $this->m_fo->set_status_proses($no_daftar, 11);

        ?>
        <script>
            window.location = 'http://<?php echo $_SERVER['SERVER_NAME'] ?>/simppadu/saved/sipb_perpanjangan.docx';
            setTimeout(function(){
	...
	public function cetak_sipb_perubahan($no_daftar){

		$this->load->library('phpword');
        $template = $this->phpword->loadTemplate(APPPATH ."../templates/sipb_perubahan.docx");
        ...
        $template->save(APPPATH. '../saved/sipb_perubahan.docx');
        $this->m_fo->set_status_proses($no_daftar, 11);

        ?>
        <script>
            window.location = 'http://<?php echo $_SERVER['SERVER_NAME'] ?>/simppadu/saved/sipb_perubahan.docx';
            setTimeout(function(){
            	window.location = '<?php echo site_url("c_cetak/belum") ?>';
            }, 3000);
        </script>
